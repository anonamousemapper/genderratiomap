...
if len(sys.argv) != 3:
  print('python -m ipum_csv_to_simplified_county_and_puma_csv ' +
        '<path/to/input_data.csv> <path/to/output_data.csv>')
  exit()

def build_simplified_row_dict(row_dict):
  simplified_row = {}
  
  # Geocode (state fip + county fip OR state fip + puma).
  # If a record has a county code (> 0), then use that to create the geo_code,
  # otherwise, fall back to puma to create the geocode.
  county_code = int(row_dict['COUNTYFIPS'])
  if county_code:
    # >= 10,000,000 indicates that a geo_code is a county.
    simplified_row[GEO_CODE] = (
      10000000 + 100000 * int(row_dict['STATEFIP']) + county_code)
    # HACK because the DC county code is wrong.
    if simplified_row[GEO_CODE] == 11100010:
      simplified_row[GEO_CODE] = 11100001
  else:
    # PUMA is a 5 digit int, hence why we multiply STATEFIP by 100000.
    simplified_row[GEO_CODE] = (
